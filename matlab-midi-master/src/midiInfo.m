function [Notes,endtime,aveMicroS] = midiInfo(midi,outputFormat,tracklist,verbose)
...
for i=1:length(tracklist)
  ...
  for msgNum=1:length(midi.track(tracknum).messages)
    ...
    %  'end of track'
    if (type==81 && midimeta==0)   
        aveMicroS = [aveMicroS,data(1)*16^4+data(2)*16^2+data(3)]; %ADDED by Ernest
    end
    if (outputFormat ~= 0)
